...
cp -f /nix/store/*-${_package}/config/log4j2.properties ${my_data}/log4j2.properties
cp -f /nix/store/*-${_package}/config/jvm.options ${my_data}/jvm.options
export ES_HOME=$(compgen -G "/nix/store/*-${_package}")
export ES_PATH_CONF=${my_data}
